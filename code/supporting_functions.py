...
def update_rover(Rover, data):
	  ...
	  # dict_keys(['samples_y', 'image', 'position', 'picking_up', 'pitch', 'speed', 'yaw', 'sample_count', 'samples_x', 'roll', 'near_sample', 'brake', 'throttle', 'steering_angle', 'fixed_turn'])
      #print(data.keys())
      ...
      print('Time =',int(Rover.total_time),'Mode =',Rover.mode, 'Unstuck Count =', Rover.unstuck_count, 'speed =',Rover.vel, 'position =', Rover.pos, 'throttle =', 
      Rover.throttle, 'steer_angle =', Rover.steer, 'Sample Found =', Rover.samples_found, 'near_sample', Rover.near_sample, 
      'picking_up', data["picking_up"], 'Start position =', Rover.start_pos, 'Yaw =', Rover.yaw, 'Home Angle =', home_angle(Rover), 'Home Distance =', distance_from_home(Rover), 'Go Home Count =', Rover.go_home_count, 'Go Home Target =', Rover.go_home_count_target)
